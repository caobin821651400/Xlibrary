...
public class XRvHorizontalProgress extends FrameLayout {
     ...
     */
    private void init() {
        LayoutInflater.from(getContext()).inflate(R.layout.layout_rv_horizontal_progrss_cblibrary, this, true);
        mProgressView = findViewById(R.id.progress_view);

        if (mBackgroundDrawable != null) {
            setBackground(mBackgroundDrawable);
        } else {
            setBackgroundResource(R.drawable.rv_indicator_bg_normal);
        }

        if (mProgressDrawable != null) {
            mProgressView.setBackground(mProgressDrawable);
        } else {
            setBackgroundResource(R.drawable.rv_indicator_bg_select);
        }
    ...
    /**
     * 设置进度填充满 可以理解max=100 progress=100
     */
