 ...
 *
 * 1.LazyThreadSafetyMode.SYNCHRONIZED:初始化属性时会有双重锁检查，保证该值只在一个线程中计算，并且所有线程会得到相同的值
 * 2.LazyThreadSafetyMode.PUBLICATION :多个线程会同时执行，初始化属性的函数会被多次调用，但是只有第一个返回的值被当做委托属性的值。
 * 3.LazyThreadSafetyMode.NONE        :没有双重锁检查，不应该用在多线程下。
 * 4.不传默认为 mode = LazyThreadSafetyMode.SYNCHRONIZED
 * ====================================================
 */
