...
CI打包：我们项目使用的jenkins，前端和后端都在用，Android这边使用的是Gradlew打包，在项目中接收一些参数，来执行不同的打包逻辑（比如，是否开启log打印，是否开启热更新功能，渠道号，测试环境还是线上环境），打包全是测试同学执行，研发不打包；
...
2.ArrayMap:
两个数组组成，一个存key的hash值，另一个存key/value，用二分法查找；扩容用array.copy 浪费空间；读写不同index时是线程安全的，读写同一个index是线程不安全的
