...
class BubbleActivity : BaseActivity() {
    ...
    private val mCustomCircleView: CustomCircleView by lazyNone { findViewById(R.id.view) }
    ...
    override fun initEvent() {
        debugBtn.setOnClickListener {
            showDebug = !showDebug
            CustomCircleView.showDebug = showDebug
        ...
        startBtn.setOnClickListener {
            ...
            if (timeText1.isNotEmpty()) {
                ...
                CustomCircleView.ALPHA_ANIM_TIME = time1
            }

            val timeText2 = timeEdit.text.toString()
            if (timeText2.isNotEmpty()) {
                val time2 = timeText2.toLong()
                CustomCircleView.TRANSLATE_ANIM_TIME = time2
            }

            val color = colorEdit.text.toString()
            if (color.isNotEmpty()) {
                try {
                    mCustomCircleView.setCircleColor(Color.parseColor("#${color}"))
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            }
            mCustomCircleView.startAnimation()
        }
        resetBtn.setOnClickListener {
            mCustomCircleView.reset(true)
        }
