...
目前有6年的工作经验，一共在两家公司呆过，在职期间，参与了多个项目研发和重构，目前主要在做一款家庭K歌软件，前期负责了项目的部分模块的搭建、项目稳定发了几版后，主要主导性能优化和性能监测方面的工作；
...
```
post方法首先判断了一个AttatchInfo变量不为空则用AttatchInfo.mHandler发送一个消息去执行Runable,View只有在dispatchAttachedToWindow方法中才对mAttatchInfo赋值，这个方法在ViewRootIml中performTraversals(吹我赛斯)调用传入的AttatchInfo
```
...
```
https://blog.csdn.net/caobin_study/article/details/115314187
https://juejin.cn/post/6844904192826425357
```
